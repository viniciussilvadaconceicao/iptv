  ...
  try {
    ...
    if(Array.isArray(dispositivos)){
      for(const d of dispositivos){
        await client.query(
          `INSERT INTO dispositivos (telefone, posicao, tipo, marca, mac, mac_maiusculo)
           VALUES ($1,$2,$3,$4,$5,$6)`,
          [tel, Number(d.slot), d.type, d.brand || null, d.mac || null, (d.macUpper || (d.mac ? String(d.mac).toUpperCase() : null))]
        );
      }
    }
  ...
  try {
    ...
    await client.query(
      `INSERT INTO planos_cliente
        (telefone, tipo_plano, qtde_telas, rotulo_duracao, dias_duracao, preco, taxa_ativacao, preco_total, inicio_em, fim_em, status)
       VALUES
...
export async function getCustomer(phone){
  ...
  const dRes = await pool.query(
    `SELECT posicao, tipo, marca, mac, mac_maiusculo
       FROM dispositivos
      WHERE telefone=$1
  ...
  return {
    ...
    plan: p ? {
      ...
      screensCount: toNumber(p.qtde_telas),
      durationLabel: p.rotulo_duracao || null,
      durationDays: toNumber(p.dias_duracao),
